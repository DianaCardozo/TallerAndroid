...
import com.example.applicationpokemon.dataBase.viewModels.FavoritePokemon
...
class ActivityFavorite : ComponentActivity(){
    private val favoritosViewModel: FavoritePokemon by lazy {
        FavoritePokemon(this)
    }
    ...
    @Composable
    fun FavoritosScreen(viewModel: FavoritePokemon) {
        val favoritos = remember { mutableStateOf<List<EntitiesPokemon>>(emptyList()) }

        var showDeleteModal by remember { mutableStateOf(false) }
        var selectedPokemon by remember { mutableStateOf<EntitiesPokemon?>(null) }
    ...
    @Composable
    fun FavoritoItem(pokemon: EntitiesPokemon, onDelete: () -> Unit) {
        Card(
            modifier = Modifier
                .fillMaxWidth()
                .padding(3.dp),
